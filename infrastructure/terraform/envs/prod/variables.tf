...
variable "private_subnet_ids" {
  description = "List of private subnet IDs for the prod environment (minimum 3 for multi-AZ)"
  type        = list(string)
}

variable "public_subnet_ids" {
  description = "List of public subnet IDs for the prod environment"
  type        = list(string)
}
...
variable "kms_key_id" {
  description = "KMS key ID for S3 SSE-KMS encryption (prod only)"
  type        = string
  default     = null
}
